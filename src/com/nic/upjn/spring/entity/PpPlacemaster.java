...
@NamedQueries({ @NamedQuery(name = "PpPlacemaster.findAll", query = "SELECT p FROM PpPlacemaster p"),
		@NamedQuery(name = "PpPlacemaster.findByPpPlacemasterid", query = "FROM PpPlacemaster WHERE ppPlacemasterid = ?"),
		...
		@NamedQuery(name = "PpPlacemaster.findByMasDistrictid", query = "FROM PpPlacemaster WHERE masDistrictid = ? order by place"),
...
public class PpPlacemaster implements Serializable {
	...
	@SequenceGenerator(name = "id_Sequence", sequenceName = "SEQ_PP_PLACEMASTER", allocationSize = 1)
	@Id
